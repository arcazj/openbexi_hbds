...
export function refreshSceneFromData(context){ if(!context) return; const {scene,setDiagramGroup,diagramGroup,setDragControls,dragControls,draggableObjects=[]}=context; if(diagramGroup){scene?.remove(diagramGroup); diagramGroup.traverse(o=>{if(o.geometry) o.geometry.dispose?.(); if(o.material) o.material.dispose?.(); if(o.isCSS2DObject) o.element?.remove?.();});}
  ...
  for(const cd of data.hypergraph.class){ if(!cd.parentClassId) continue; const p=modelRuntime.classById.get(cd.parentClassId), ch=modelRuntime.classById.get(cd.id); if(p&&ch){const wp=ch.getWorldPosition(new THREE.Vector3()); p.worldToLocal(wp); dg.remove(ch); ch.position.copy(wp); p.add(ch);} }
  ...
  draggableObjects.length=0; for(const cd of data.hypergraph.class){ if(!cd.parentClassId){const o=modelRuntime.classById.get(cd.id); if(o) draggableObjects.push(o);} }
