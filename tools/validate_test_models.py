...
def expected_name(path: Path) -> str:
    return " ".join(path.stem.replace("_", " ").replace("-", " ").split()).title()
...
def validate_metadata(path: Path, data: dict):
    ...
    if not isinstance(name, str) or not name.strip():
        errors.append(f"{path.name}: metadata.name must be a non-empty string")
    elif name != expected_name(path):
        errors.append(f"{path.name}: metadata.name should be '{expected_name(path)}', got '{name}'")

    purpose = metadata.get("purpose")
    if not isinstance(purpose, str) or not purpose.strip():
        errors.append(f"{path.name}: metadata.purpose must be a non-empty string")
...
def validate_model_ids(path: Path, data: dict, seen_globally: dict[str, list[str]]):
    ...
    if not isinstance(classes, list):
        return [f"{path.name}: hypergraph.class must be an array"]
...
def validate_global_ids(seen_globally: dict[str, list[str]]):
    ...
    for entity_id, owners in sorted(seen_globally.items()):
        ...
        if len(files) > 1:
            errors.append(f"duplicate global test-model id '{entity_id}' appears in {', '.join(files)}")
    return errors
...
def main() -> int:
    ...
    if errors:
        print("Test model validation failed:")
        for error in errors:
            print(f"- {error}")
        return 1
    print("Test model validation passed.")
    return 0
